...
`scripts/sync-skills.sh` (run automatically by `sync-dotfiles`/`bootstrap.sh`, and daily via `morning`) symlinks each folder here into `~/.claude/skills/<name>` and `~/.cursor/skills/<name>`, and removes the symlink again once the folder is deleted from its source repo or the repo is unlisted. dotfiles-arch is always synced first; register extra repos with `sync-sources add /path/to/repo` (later sources override on name collision). It never touches anything else already in those directories — only managed symlinks under `{source}/skills/`.

...
# Maintenance (helpers live in ~/.local/bin, all prefixed dela- for discoverability)
alias check='dela-check-dotfiles'
alias orphans='dela-remove-orphans'

# Jump to a repo with fzf: r → cd "$(dela-repos)"
if command -v fzf &> /dev/null; then
    alias r='cd "$(dela-repos)"'
fi
...
packages() {
    if [ -f "$HOME/.packages.md" ]; then
        ...
        echo "Package catalog not found at $HOME/.packages.md (run dela-sync-dotfiles to link it)"
    fi
}

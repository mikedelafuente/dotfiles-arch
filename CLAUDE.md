...
- **sync-skills.sh**: Symlinks each skill folder from dotfiles-arch and any extra sources listed in `~/.config/dotfiles-arch/sync-sources` into `~/.claude/skills/<name>` and `~/.cursor/skills/<name>` (both tools consume the same `SKILL.md` format — Claude Code's own skills scan is not recursive, so each skill is linked individually rather than as one nested folder); later sources override on name collision; prunes managed symlinks when the source is deleted, unlisted, or a skill vanishes upstream. Only ever touches symlinks whose target is a configured source's effective skills dir — real entries elsewhere are never touched. Runs via `post-link-hooks.sh` (so every `bootstrap.sh`/`sync-dotfiles`) and as a step in `morning`; also runnable directly as `sync-skills`. Manage extra sources with `sync-sources`
- **sync-rules.sh**: Symlinks each `*.mdc` rule file from dotfiles-arch and any extra sources listed in `~/.config/dotfiles-arch/sync-sources` into `~/.cursor/rules/<name>.mdc` (Cursor only — Claude Code has no equivalent auto-loaded rules directory); later sources override on name collision; prunes managed symlinks when the source is deleted, unlisted, or a rule vanishes upstream. Only ever touches symlinks whose target is a configured source's effective rules dir — real entries elsewhere are never touched. Runs via `post-link-hooks.sh` (so every `bootstrap.sh`/`sync-dotfiles`) and as a step in `morning`; also runnable directly as `sync-rules`. Manage extra sources with `sync-sources`
- **sync-sources.sh**: List/add/remove extra rules/skills sources (`sync-sources list|add|remove`); dotfiles-arch is always primary. Each source has a type (`sync-sources add /path --type <type>`): `standard` (default) expects `rules/` and/or `skills/` subdirs under the given path, same layout as dotfiles-arch itself; `skills-root` treats the given path itself as a flat folder of skill dirs (no `skills/` subdir) — for pulling in one subfolder of someone else's skills repo, e.g. `sync-sources add ~/repos/mattpocock/skills/skills/engineering --type skills-root`; `rules-root` is the same idea for a flat folder of `*.mdc` files. A single upstream repo with several such subfolders (e.g. `mattpocock/skills/skills/{engineering,misc,productivity}`) needs one `--type skills-root` entry per subfolder. Config: `~/.config/dotfiles-arch/sync-sources` (lines are `path` for standard, or `type:path` for other types)

...
- **update-npm-clis.sh**: Runs `npm update -g` for each of `claude`/`codex`/`pi` that's actually installed (mapping harness id to its npm package: `@anthropic-ai/claude-code`, `@openai/codex`, `@earendil-works/pi-coding-agent`) — `setup-claude.sh`/`setup-codex.sh`/`setup-pi.sh` only `npm install -g` these when missing, so without this they'd never update in place after the initial install. `opencode` is deliberately excluded: it's a pacman package (`setup-opencode.sh`), already refreshed by `dfa-update-system`. Skips quietly (exit 0) if NVM/npm isn't available. Also runnable directly as `dfa-update-npm-clis`, and included as a step in `dfa-daily`
...
- **sync-skills.sh**: Symlinks each skill folder from dotfiles-arch and any extra sources listed in `~/.config/dotfiles-arch/sync-sources` into `~/.claude/skills/<name>`, `~/.cursor/skills/<name>`, and `~/.pi/agent/skills/<name>` (all three tools consume the same `SKILL.md` format — Claude Code's own skills scan is not recursive, so each skill is linked individually rather than as one nested folder; Pi discovers `~/.pi/agent/skills` natively); later sources override on name collision; prunes managed symlinks when the source is deleted, unlisted, or a skill vanishes upstream. Also prunes the legacy `~/.claude/skills` / `~/.codex/skills` entries from `~/.pi/agent/settings.json` — Pi used to be pointed at those harness dirs explicitly, and now that the same skills are mirrored into Pi's own dir, leaving them would make Pi load every skill twice and warn about name collisions. Only ever touches symlinks whose target is a configured source's effective skills dir — real entries elsewhere are never touched (and only those two exact settings entries, never user-added ones). Runs via `post-link-hooks.sh` (so every `bootstrap.sh`/`dfa-sync-dotfiles`) and as a step in `dfa-daily`; also runnable directly as `dfa-sync-skills`. Manage extra sources with `dfa-sync-sources`
- **sync-rules.sh**: For each source (dotfiles-arch + extras from `~/.config/dotfiles-arch/sync-sources`), first builds a normalized copy of its rules into `~/.config/dotfiles-arch/rules-build/<slug>/` (`build_sync_source_rules` in `sync-sources-lib.sh`) — ready-made `.mdc` files are copied as-is, plain `.md` files with YAML frontmatter are auto-converted (a `README.md` is never treated as a rule) — then symlinks each `.mdc` from there into `~/.cursor/rules/<name>.mdc`; later sources override on name collision. Rules with `alwaysApply: true` are also concatenated (frontmatter-stripped body only) into a per-source `claude-rules.md` in that same build dir, symlinked to `~/.claude/dfa-rules-<slug>.md`, with a matching `@dfa-rules-<slug>.md` import line idempotently appended to `~/.claude/CLAUDE.md` — giving Claude Code parity with Cursor's rules, which it previously had none of. Pi has no `@import` mechanism, so instead all sources' alwaysApply bodies are concatenated into one generated `rules-build/pi-agents.md`, symlinked to `~/.pi/agent/AGENTS.md` (Pi's global agent file); when no source has alwaysApply rules the stale link is pruned. Any pre-existing real (non-symlinked) file at a managed target name is deleted and replaced by the symlink (e.g. a leftover copy from another tool's installer), except a user-written `~/.pi/agent/AGENTS.md` without the dfa-sync-rules marker, which is left alone with a warning. Prunes managed symlinks/import lines when a source is deleted, unlisted, or a rule/import vanishes upstream. Only ever touches entries whose target is a configured source's effective dir — real entries elsewhere are never touched. Runs via `post-link-hooks.sh` (so every `bootstrap.sh`/`dfa-sync-dotfiles`) and as a step in `dfa-daily`; also runnable directly as `dfa-sync-rules`. Manage extra sources with `dfa-sync-sources`

...
- `FULL_NAME`, `EMAIL_ADDRESS`, `SETUP_PROFILES` (space-separated multi-select),
  ...
  `DEFAULT_AGENT` (`claude`|`codex`, resolved by `setup-code.sh` — see below)
...
Shared: Kitty, tmux, Claude Code, Codex, Ollama, Neovim, languages, Docker, Spotify, Obsidian, GNOME/Pop Shell, etc. `run-profile-setup.sh` runs `setup-code.sh` last, after all profile extras, so any profile-installed tools are already on PATH.
